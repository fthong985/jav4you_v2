  ...
  return (
    <div className="w-full my-10 flex flex-col items-center space-y-5 ">
              ...
              {typeof pageNumber === "number" ? (
                <PaginationLink
                  className={`text-white ${
                    currentPage === pageNumber && "text-white bg-main border-0"
                  }`}
                ...
                <span className="px-2 text-white">...</span> // Ellipsis for skipped pages
              )}
            </PaginationItem>
          ))}
      ...
      <form
        ...
        className="w-fit text-white bg-[#3b4252] border border-[#4c566a] py-1 px-2 flex items-center gap-2 rounded-lg"
      >
        <input
          ref={inputRef}
          type="number"
          ...
          className="max-w-10 text-xl bg-transparent border-0 text-white focus:outline-0 appearance-none hover:appearance-none  "
        />
        <span>/ {totalPage}</span>
